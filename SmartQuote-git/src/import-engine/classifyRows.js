...
export function classifyRow(row, opt = {}) {
  ...
  // 1. STRONG SKIP: điều khoản/giao hàng/bảo hành/thanh toán không phải sản phẩm.
  ...
  const money = hasMoneyValue(row, priceCol);
  // Báo giá cũ hay có dòng tổng nhóm kiểu "IV/ Hệ thống mạng..." kèm thành tiền.
  // Dòng này là hạng mục/subtotal, không phải catalog product.
  if (isLikelyOldQuoteSectionRow(joined)) return ROW_CLASS.SECTION;
