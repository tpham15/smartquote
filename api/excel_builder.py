import json, sys, io, requests, socket, ipaddress
...
def fetch_img(url, size=(56,56)):
    """Tải ảnh công khai → openpyxl Image object. Trả None nếu lỗi/URL nội bộ/quá lớn."""
    if not url or "encrypted-tbn" in url or not _is_public_image_url(str(url)):
        return None
    try:
        ...
        for chunk in r.iter_content(32 * 1024):
            ...
            if total > max_bytes:
                return None
            chunks.append(chunk)
        img = PILImage.open(io.BytesIO(b"".join(chunks))).convert("RGB")
...
def build_excel(data: dict) -> bytes:
    ...
    row_h(R,28)
    mr(R,1,10,
       company.get("introText") or "Xin trân trọng gửi tới Quý Khách hàng Bảng báo giá với những chi tiết như sau:",
    ...
    # Nhân công
    nc_pct = float(company.get("laborPercent",0)) / 100
    nc_r = R
    row_h(R,16)
    mr(R,1,10, f"Nhân công, lắp đặt & lập trình hệ thống ({company.get('laborPercent',0)}%):",
...
if __name__ == "__main__":
    # Fixture tổng hợp, không chứa dữ liệu của bất kỳ đại lý/khách hàng thật nào.
    data = {
        "company": {
            "name": "CÔNG TY DEMO",
            "address": "Địa chỉ mẫu",
        ...
        "customer": {
            ...
            "phone": "0900000001",
            "address": "Khu vực mẫu",
            "quoteNumber": "BG-DEMO-001",
            "category": "Hạng mục mẫu",
        },
        "rooms": [
            {
                "name": "I./ Hạng mục mẫu",
                "lines": [
                    {"productId":"p1","qty":2,"price":100000,"note":"Khu vực A",
                     "product":{"name":"Thiết bị mẫu A","sku":"DEMO-001","supplier":"Nhà cung cấp mẫu",
                                "unit":"Cái","specs":"Thông số mẫu","image":""}},
                    {"productId":"p2","qty":1,"price":200000,"note":"Khu vực B",
                     "product":{"name":"Thiết bị mẫu B","sku":"DEMO-002","supplier":"Nhà cung cấp mẫu",
                                "unit":"Cái","specs":"Thông số mẫu","image":""}},
                ],
            },
        ],
    ...
    with open("/tmp/smartquote_demo.xlsx", "wb") as f:
        f.write(out)
    print("Wrote /tmp/smartquote_demo.xlsx")
