...
async function runGuardrails() {
  ...
  const sub = rowToItem(subtotalRow, { name: 1, price: 5 }, 'BÁO GIÁ', '', 'Nhà cung cấp mẫu');
  ...
  const real = rowToItem(realRow, { name: 1, sku: 2, specs: 3, price: 5 }, 'BÁO GIÁ', '', 'Nhà cung cấp mẫu');
...
async function runFile(filePath) {
  ...
  // File-specific dealer fixtures were intentionally removed for white-label distribution.
  return result;
}
