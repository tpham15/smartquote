...
function runPdfGuardrails() {
  ...
  const prices = extractMoneyValues('Thiết bị mẫu DEMO-01 12.340.000 5.670.000');
  assert(prices.length >= 2, 'must extract multiple prices from a Bisco-like line');
  const picked = pickPriceFields(prices);
  assert(picked.costPrice === 5670000, `costPrice should be lowest sensible price, got ${picked.costPrice}`);
  assert(picked.listPrice === 12340000, `listPrice should be highest, got ${picked.listPrice}`);
  ...
  const lightingVariants = normalizePdfItems([
    {
      ...
      rawText: 'STT 1 LM-ST7-55-O 650,000 820,000 1,090,000',
      sourcePage: 1,
      sourceRow: 1,
    },
    {
      ...
      rawText: 'STT 1 LM-ST7-55-D 650,000 820,000 1,090,000',
      sourcePage: 1,
      sourceRow: 1,
    },
    {
      ...
      rawText: 'STT 1 LM-ST7-55-T 650,000 820,000 1,090,000',
      sourcePage: 1,
      sourceRow: 1,
    },
  ...
  assert(collapsedLighting[0].costPrice === 650000, `collapsed row should use lowest clear variant price, got ${collapsedLighting[0].costPrice}`);
...
const LUMI_PAGE = {
  ...
  text: [
    ...
    'Công tắc Luto 2 nút LM-S2N/S Màu trắng/đen 2.018.000',
    'Công tắc Luto 3 nút LM-S3N/S 2.133.000',
    'Ổ CẮM',
    'Ổ cắm Luso viền thẳng champagne LM-SK4/SC 621.000',
    'Bộ điều khiển trung tâm LM-HC/4.0 Nguồn cấp: 220VAC 3.010.000',
...
const BISCO_PAGE = {
  ...
  text: [
    ...
    '1 22F005 Cổng mở xoay âm sàn VULCAN 24V 600kg Bộ 42.100.000 19.765.000',
