import { tenantStorageGetItem, tenantStorageSetItem, tenantStorageRemoveItem } from "../storage/tenantStorage.js";
...
export function loadCorrectionLearning() {
  try {
    const parsed = JSON.parse(tenantStorageGetItem(LEARNING_KEY) || "null");
...
function saveDb(db) {
  try {
    db.updatedAt = now();
    pruneDb(db);
    return tenantStorageSetItem(LEARNING_KEY, JSON.stringify(db));
  } catch {
    return false;
  }
}
...
export function clearCorrectionLearning() {
  try { return tenantStorageRemoveItem(LEARNING_KEY); } catch { return false; }
}
