...
export async function runImport(file, ctx = {}) {
  ...
  if (needsAIFallback(determResult) && typeof ctx.aiExtract === "function") {
    // gom các sheet để AI đọc lại (chỉ sheet có vấn đề). Có trần để tránh workbook nhiều sheet đốt quota.
    let aiFallbackSheets = 0;
    for (const ps of perSheet) {
      if (aiFallbackSheets >= (ctx.maxAIFallbackSheets || MAX_AI_FALLBACK_SHEETS_PER_IMPORT)) {
        warnings.push(`AI fallback đã dừng sau ${aiFallbackSheets} sheet khó để tránh tốn quota; hãy kiểm tra các sheet còn lại thủ công.`);
        break;
      }
      ...
      if (badRatio > 0.4 || sheetItems.length === 0) {
        ...
        if (aiItems && aiItems.length) {
          ...
          const aiRows = new Set(aiScored.map((it) => Number(it.source?.rowIndex)).filter((n) => Number.isInteger(n) && n >= 0));
          const shouldKeepDeterministic = (it) => {
            if (it.source?.sheet !== ps.sheet.name) return true;
            // Giữ các dòng deterministic đã đúng; AI chỉ thay dòng review/rejected hoặc dòng trùng rowIndex.
            if (it.status !== STATUS.REVIEW && it.status !== STATUS.REJECTED) return true;
            if (aiRows.size && !aiRows.has(Number(it.source?.rowIndex))) return true;
            return false;
          };
          scored = scored.filter(shouldKeepDeterministic).concat(aiScored);
        }
      }
    }
  }
