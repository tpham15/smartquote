...
export function scoreConfidence(items, mapConfidence = 0.5) {
  return items.map((item) => {
    ...
    const requiresPriceReview = (item.issues || []).some((i) => ["price_column_uncertain", "price_scale_suspect"].includes(issueCode(i)));

    if (shouldSkipSubtotal) {
      status = STATUS.SKIPPED || "skipped";
    } else if (hardReject || (!item.price && !item.sku)) {
      status = STATUS.REJECTED;
    } else if (item.matchedProductId && conf >= 0.7 && !requiresPriceReview) {
      status = STATUS.MATCHED;
    } else if (requiresPriceReview || conf < 0.55 || blockingIssues.some((i) => issueLevel(i) === "error") || blockingIssues.length >= 2) {
