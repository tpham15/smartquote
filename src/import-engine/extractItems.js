...
/** Parse số tiền từ chuỗi, trả 0 nếu không hợp lệ (>=100), có hỗ trợ scale từ header: (1.000đ), (triệu). */
function parseDecimalForScaledColumn(s) {
...
function parsePrice(s, scale = 1) {
  const safeScale = Number(scale) || 1;
  let base = parseSafePrice(s);
...
function priceScaleForCol(map, col) {
  ...
  return Number(map?._priceScaleByCol?.[col] || (col === map?.price ? map?._priceScale : 1) || 1);
}

function priceScaleForKey(map, key) {
  const col = map?.[key];
  return priceScaleForCol(map, col);
}

function maybePushPriceScaleIssues(issues, map, price) {
  const scale = Number(map?._priceScale || 1);
  if (scale > 1) {
    issues.push(issue(
      "price_scaled_from_header",
      "info",
      `Đã nhân giá theo nhãn cột (${scale.toLocaleString("vi-VN")}×): ${map._priceSourceLabel || "cột giá"}`,
      "costPrice"
    ));
  }
  if (scale > 1 && price > PRICE_SCALE_SUSPECT_LIMIT) {
    issues.push(issue(
      "price_scale_suspect",
      "warning",
      "Giá sau khi nhân đơn vị cột vượt 1 tỷ, cần xác nhận lại đơn vị giá",
      "costPrice",
      "Kiểm tra xem cột giá đang tính theo đồng, nghìn đồng hay triệu đồng"
    ));
  }
  if (map?._priceColUncertain) {
    issues.push(issue(
      "price_column_uncertain",
      "warning",
      `Chưa chắc cột đang dùng là giá mua vào${map._priceSourceLabel ? `: ${map._priceSourceLabel}` : ""}`,
      "costPrice",
      "Xác nhận lại cột GIÁ MUA VÀO trước khi lưu catalog"
    ));
  }
}
...
function priceCandidatesFromCols(row, cols = [], map = {}) {
  return [...new Set(cols || [])]
    .map((c) => parsePrice(row.text[c], priceScaleForCol(map, c)))
    .filter((n) => n > 0);
}
...
function rowToItem(row, map, sheetName, sectionName, fileSupplier) {
  ...
  const issues = [];
  let price = parsePrice(get("price"), priceScaleForKey(map, "price"));
  const oldListPrice = parsePrice(get("listPrice"), priceScaleForKey(map, "listPrice"));
  const currentListPrice = parsePrice(get("currentListPrice"), priceScaleForKey(map, "currentListPrice"));
  ...
  if (listPrice === 0 && Array.isArray(map._otherPriceCols)) {
    const candidates = map._otherPriceCols.map((c) => parsePrice(row.text[c], priceScaleForCol(map, c))).filter((n) => n > 0);
    if (candidates.length) listPrice = Math.max(...candidates);
  }

  // Giá bán lẻ thấp nhất/MAP chỉ để lưu tham khảo, không áp hệ số cứng.
  ...
  if (kind === "service") {
    ...
    issues.push(issue("billable_service_item", "info", "Hạng mục dịch vụ/nhân công được giữ trong báo giá", "name"));
  }
  ...
  return {
    ...
    sku,
    kind,
    category: kind === "service" ? (earlyCategory || "Dịch vụ / Nhân công") : earlyCategory,
